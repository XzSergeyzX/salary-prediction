...
print("=== Reading data ===")
chunk_size = 10000  # Increase chunk size for potentially more data
with pd.read_csv(config.data_path, chunksize=chunk_size) as reader:
    df = next(reader)
...
X = df.drop([config.target_column, 'salary_in_usd'], axis=1, errors='ignore')
